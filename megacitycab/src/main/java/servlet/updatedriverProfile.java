...
public class updatedriverProfile extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if (loggedInDriver == null) {
            response.sendRedirect("driverlogin.jsp");  // Redirect if session is invalid
            return;
        }
        ...
        if (isUpdated) {
            ...
            response.sendRedirect("driverprofile.jsp?success=true");
        } else {
            response.sendRedirect("driverprofile.jsp?error=true");
        }
    }
}
